...
const { dirname, join } = lazyReq('path')('dirname', 'join');
...
const task = new Task(join(__dirname, 'worker.js'));
...
function endMeasure(baseName) {
  if (atom.inDevMode()) {
    ...
    if (performance.getEntriesByName(`${baseName}-start`).length > 0) {
...
function parseResults(editor, filePath, lintResults) {
  startMeasure('linter-stylelint: Parsing results');

  if (lintResults.error) {
    // Was it a code parsing error?
    if (lintResults.error.line) {
      endMeasure('linter-stylelint: Parsing results');
      endMeasure('linter-stylelint: Lint');
      return [{
        type: 'Error',
        severity: 'error',
        text: lintResults.error.reason || lintResults.error.message,
        filePath,
        range: createRange(editor, lintResults.error)
      }];
    }
    ...
    atom.notifications.addError('Unable to run stylelint', {
      detail: lintResults.error.reason || lintResults.error.message,
      dismissable: true
    });
  ...
  const result = lintResults.data.results.shift();
...
function runStylelint(editor, options, filePath) {
  return new Promise(resolve => {
    startMeasure('linter-stylelint: Task');
    startMeasure('linter-stylelint: Results');
    if (!taskStarted) {
      task.start();
      taskStarted = true;
    }
    const evtSub = task.on('linter-stylelint:results', data => {
      evtSub.dispose();
      resolve(parseResults(editor, filePath, data));
      endMeasure('linter-stylelint: Results');
    });
    task.send(options);
    endMeasure('linter-stylelint: Task');
  });
}
